...
*************************/
 /* var win = $(window),
      foo = $('#typer');
      foo.typer(['<h2>Художник</h2>','<h2>Декоратор </h2>', '<h2> Дизайнер-практик</h2>', '<h2> Модельер</h2>', '<h2> Преподаватель</h2>' ]);
            win.resize(function(){
...
var ww = $(window).width(); //Ширина браузера
var wh = $(window).height(); //Высота браузера


if(ww>=wh) $('.center img').css({'width':'auto','height':'89%'});
else $('.center img').css({'width':'89%','height':'auto'});
...
if(ww>=wh) $('.center img').css({'width':'auto','height':'95%'});
else $('.center img').css({'width':'95%','height':'auto'});
